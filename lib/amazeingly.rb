...
module Amazeingly
  class Amazeingly
    ...
    def start
      raise InputException, 'You must provide objects to search' if objects.empty?
    ...
    def read_file(file_path)
      ...
      raise InputException, 'Cannot read the specified input file'

    rescue JSON::ParserError
      raise InputException, 'The specified file does not contain a valid JSON'
    end

    def create_rooms_collection(rooms)
      RoomsCollection.new(rooms)
    end

    def search_path(rooms_collection)
      path_finder = PathFinder.new(rooms_collection: rooms_collection)
